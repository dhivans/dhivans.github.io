...
| 5 | Buying guides & comparisons (§4.1) | Highest intent-to-purchase content, cheapest to produce even though writing isn't the favourite part | 🟡 (two guides live 2026-09-03: D2F-01 vs D2F-5L microswitches, and What the BMP280 Actually Measures — the latter also caught and fixed a real lingering "measures humidity" error in the BMP280 product page's body copy) |
| 6 | Internal linking | Free, compounds every guide written | 🟡 (the D2F guide already backlinks from both product pages via the existing "Featured In" mechanism — confirms it works on real content) |
| 7 | Rest of the product page standard (§2): cross-sell links | "Commonly used with" turns single-item sales into multi-item ones — free once written | ✅ (done 2026-09-03: all 46 products populated with real, project-context `related:` relationships — not category-matched — verified live, "Pairs Well With" rendering correctly and symmetrically) |
| 7b | Real (DST-taken) product photography | Differentiates from the wholesaler's own stock image — see §2 | ⬜ (needs camera + physical products, same blocker as #4 — automated image *sync* from Amazon's own listing photos already exists via `scripts/sync_products.py`, but that's the baseline being replaced, not this item) |
